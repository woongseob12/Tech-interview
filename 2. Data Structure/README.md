...
`동일한 자료형의 데이터가 일렬로 나열된 자료구조`

- 연속된 메모리 공간에 순차적으로 저장
- 인덱스를 통해 O(1)의 시간 복잡도로 원소에 접근 가능
- 정의와 동시에 크기를 지정하며, 크기를 바꿀 수 없음

---

## 선형리스트(ArrayList)

![image](https://user-images.githubusercontent.com/55429912/122240002-5f3f5a80-cefc-11eb-8f32-821645bbc3b4.png)
...
- 선언시 사용할 객체의 타입을 제네릭(<>)을 통해 지정해주는것이 일반적
   1. 타입의 안정성: 의도하지 않은 타입의 객체가 저장되는 것을 막음
   2. 불필요한 형변환을 줄여 코드의 간결함
- 순차리스트
- 인덱스로 내부의 객체 관리
- Object만 담을 수 있음(Primitive data types을 가질 수 없음)
...
- 각 노드는 데이터와 다음 노드를 가르키는 포인터로 이루어짐
- 데이터를 검색 시 처음 노드부터 순회해야함(O(N)의 시간 복잡도)
- 메모리 공간 상에서 각 노드들이 불연속적으로 저장됨
...
| :--------: | :--: | :---------------------: | :---------------------: |
|   Array    | O(1) | - | - |
|   ArrayList    | O(1) | O(N)/ 맨 뒤: O(1) | O(N)/ 맨 뒤: O(1) |
| LinkedList | O(N) | O(N)/ 맨 앞: O(1) | O(N)/ 맨 앞: O(1) |
...
- ArrayList
  - 정적 + 동적 메모리
  - 정적으로 Array처럼 선언하고, 후에 추가적인 공간이 필요할 경우 동적으로 늘림
...
- 콜스택, 문자열 역순 출력 등에 사용
  ...
  - 서브루틴은 완료후에는 호출된 지점으로 제어를 넘겨야하기 때문에, 서브루틴 호출시 콜스택에 반환 주소를 제일 먼저 집어 넣음
