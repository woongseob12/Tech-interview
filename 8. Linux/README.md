...
![image](https://user-images.githubusercontent.com/55429912/122541673-df80cf80-d064-11eb-858f-36d280d46739.png)

| 레벨   | 방식                                                                                                                                                                                                                     |
| ------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RAID 0 | - 스트라이핑 저장 방식: 연속된 데이터를 여러 디스크에 나눠 저장<br>- 최소 2개의 하드 디스크 필요<br>- 입출력 작업이 모든 디스크에 동시에 진행<br>- 고장 대비 능력이 없어 주요 데이터 저장은 부적합                       |
| RAID 1 | - 미러링 방식: 하나의 디스크에 데이터를 저장하면 다른 디스크에 동일한 내용이 백업되어 저장<br>- 데이터 저장 시 두 배의 용량이 필요<br>- 결함 허용을 제공하지만 공간 효율성은 떨어짐<br>- 주요한 데이터를 저장하기에 적합 |
| RAID 5 | - 스트라이핑 저장 방식<br>- 패리티(Parity)를 이용해서 데이터를 복구<br>- 디스크가 2개 이상 고장나면 패리티로도 복구 불가                                                                                                 |
| RAID 6 | - 2개의 패리티 사용<br>- 안정성은 높지만 공간효율과 성능은 낮아짐                                                                                                                                                        |

![image](https://user-images.githubusercontent.com/55429912/122545270-9e8aba00-d068-11eb-800d-27477d112519.png)

|               |                                                                                                                 |                                                                          |
| :-----------: | :-------------------------------------------------------------------------------------------------------------: | :----------------------------------------------------------------------- |
| RAID<br>0 + 1 | ![image](https://user-images.githubusercontent.com/55429912/122546394-dc3c1280-d069-11eb-9bb9-b0c0a48b5859.png) | - 스트라이핑 ⇒ 미러링<br>- 속도는 빠르나 데이터 복구 시간이 오래 걸림    |
| RAID<br>1 + 0 | ![image](https://user-images.githubusercontent.com/55429912/122546500-fc6bd180-d069-11eb-96ac-0c096bda13f4.png) | - 미러링 ⇒ 스트라이핑<br>- 손실된 데이터만 빠른 복원이 가능(운영상 유리) |
...
1. 명령어 해석기
   ...
   - 커널과 사용자간의 대화식 인터페이스를 제공
2. 로그인할 때 실행되어 사용자별로 사용 환경 설정을 가능하게 한다.
3. 스크립트 언어
   - 쉘 자체가 프로그래밍 기능을 갖고 있음

로그인 쉘(Login Shell): 로그인할 때 수행되는 **기본 쉘**

- `logout` 명령을 통해 쉘을 종료

비로그인 쉘(Non-login Shell): 로그인 없이 실행되는 쉘

- GUI 환경에서 새 터미널을 여는 경우
- `sudo`, `su`를 사용할 경우
...
- 현재 쉘의 자식 프로세스로 실행
- `exit` 명령을 통해 서브 쉘을 종료하고 부모 쉘로 돌아감
...
|                                     |                                                                                                                                                                                                                         |
| :---------------------------------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 포그라운드 프로세스<br>(Foreground) | - 사용자와 상호작용하는 프로세스<br>- 터미널에 직접 연결되어 입출력을 주고받는 프로세스<br>- 명령 입력 후 수행 종료까지 기다려야하는 프로세스<br>- 화면에 실행되는 것이 보이는 프로세스<br> ex. 응용프로그램, 명령어 등 |
| 백그라운드 프로세스<br>(Background) | - 사용자와 직접적인 대화를 하지 않고 뒤에서 실행되는 프로세스<br>- 사용자의 입력에 관계없이 실행되는 프로세스<br>- 실행은 되지만 화면에 나타나지 않고 실행되는 프로세스<br> ex. 시스템 프로그램, 데몬 등                |
...
- 기존 프로세스를 새로운 프로세스로 변경
- 새로운 프로세스를 위한 메모리를 할당X

### 데몬(Daemon)

`Linux에서 부팅 시 자동으로 실행되는 백그라운드 프로세스`
...
1. standalone
   ...
   - 빠른 응답속도
   - 메모리에 항상 상주해 있으므로 메모리 점유로 인한 서버 부하
2. xinetd

- 슈퍼 데몬에 의해 관리
- 필요한 경우에만 메모리에 적재되어 실행
- 느린 응답속도
- 필요한 경우에만 적재되어 서버 부하 감소
